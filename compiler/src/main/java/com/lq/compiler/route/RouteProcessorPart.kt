...
import com.lq.compiler.util.getGroupName
import com.lq.compiler.util.getModuleNameCapitalize
import com.lq.compiler.util.getRootName
...
internal class RouteProcessorPart(
    ...
    override fun process(resolver: Resolver) {
        ...
        resolver
            .getSymbolsWithAnnotation(Const.RouteQualifiedName)
    ...
    private fun generateRoot(routeMap: Map<String, Map<String, String>>) {
        ...
        val function =
            FunSpec.builder("loadInto").addModifiers(KModifier.OVERRIDE).addParameter(
                "rootMap",
        ...
        val classInfo =
            TypeSpec
                ...
                .addSuperinterface(rootInterface)
                .addFunction(function.build())
                .build()

        val fileSpec =
            FileSpec
                .builder(Const.H_ROUTER_PACKAGE, rootClassName)
                .addType(classInfo)
                .build()
        ...
        for ((groupName, pathMap) in groupMap) {
            ...
            val function =
                FunSpec
                    ...
                    .addParameter(
                        "groupMap",
                        Const.MutableMapClassName.parameterizedBy(
                            Const.StringClassName,
                            routeMetaClass,
                        ),
                    )
//                .addParameter("path",Const.StringClassName)
            val safeLoadPathMember =
                MemberName(
...
//                logger.warn("generateGroup groupName:$groupName className:$className path:$path classInfo:$classInfo")
            ...
            val fileSpec =
                FileSpec
                    .builder(Const.H_ROUTER_PACKAGE, className)
                    .addType(
                        TypeSpec
                            .classBuilder(className)
                            .addSuperinterface(groupInterface)
                            .addFunction(function.build())
                            .build(),
                    ).build()
